...
-- ==============================================================
-- Public function: open lookup window and return selected id
-- ==============================================================
FUNCTION get_list() RETURNS STRING
    ...
    DIALOG ATTRIBUTES(UNBUFFERED)
        ...
        DISPLAY ARRAY debtor_list TO r_debtors_list.*
            ...
            ON ACTION ACCEPT
                LET selected_id = get_selected_debtor(debtor_list)
...
FUNCTION get_selected_debtor(
    ...
    IF current_row >= 1 AND current_row <= debtor_list.getLength() THEN
        RETURN debtor_list[current_row].id
    END IF
    RETURN NULL
END FUNCTION
...
FUNCTION load_debtors_for_lookup(
    ...
    PREPARE stmt
        FROM "SELECT id, cust_name, status, "
            || "FROM dl01_mast "
