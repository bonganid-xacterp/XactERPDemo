...
FUNCTION dp_list_all() RETURNS DYNAMIC ARRAY OF t_debtor
    ...
    DECLARE c CURSOR FOR
        SELECT acc_code, cust_name, phone, email,
               address1, address2, address3,
               cr_limit, balance, status
          FROM dl01_mast
         ORDER BY acc_code
...
FUNCTION dp_get_by_code(p_code STRING)
    ...
    DECLARE c_debtor CURSOR FOR
        SELECT * FROM dl01_mast WHERE acc_code = p_code
...
FUNCTION dp_insert(d t_debtor) RETURNS SMALLINT
    INSERT INTO dl01_mast
        (acc_code, cust_name, phone, email,
         ...
         cr_limit, balance, status)
    VALUES
        (d.acc_code, d.cust_name, d.phone, d.email,
         d.address1, d.address2, d.address3,
         d.cr_limit, d.balance, d.status)
