...
DEFINE m_arr_ord_full_lines DYNAMIC ARRAY OF RECORD LIKE sa31_ord_det.*
...
FUNCTION new_order()
    ...
    INPUT BY NAME l_hdr.cust_id, l_hdr.trans_date, l_hdr.ref_doc_type,
        ...
        BEFORE INPUT
            ...
            MESSAGE SFMT("Enter order header details for Order #%1", l_next_doc_no)
...
FUNCTION input_order_lines(p_hdr_id INTEGER)
    ...
    OPEN WINDOW w_order_lines WITH FORM "sa131_order" ATTRIBUTES(STYLE="dialog")
    ...
    DIALOG ATTRIBUTES(UNBUFFERED)
        ...
        DISPLAY ARRAY m_arr_ord_lines TO arr_sa_ord_lines.*
            ...
            ON ACTION delete ATTRIBUTES(TEXT="Delete Line", IMAGE="delete")
                IF arr_curr() > 0 THEN
                    CALL delete_order_line(p_hdr_id, arr_curr())
                    CALL calculate_order_totals()
                END IF
            ...
            ON ACTION save ATTRIBUTES(TEXT="Save Lines", IMAGE="save")
                ...
                CALL utils_globals.show_success("Order lines saved successfully.")
                EXIT DIALOG

            ON ACTION close ATTRIBUTES(TEXT="Close", IMAGE="exit")
                IF m_arr_ord_lines.getLength() > 0 THEN
...
FUNCTION edit_or_add_order_line(p_doc_id INTEGER, p_row INTEGER, p_is_new SMALLINT)
    ...
    IF p_is_new THEN
        ...
        LET l_line.line_no = m_arr_ord_full_lines.getLength() + 1
    ...
    ELSE
        ...
        LET l_line.* = m_arr_ord_full_lines[p_row].*
    END IF

    -- ==============================
    -- Input Dialog for editing/adding
    -- ==============================
    OPEN WINDOW w_line_edit WITH FORM "sa131_order_line" ATTRIBUTES(STYLE="dialog")
    ...
    INPUT BY NAME l_line.stock_id, l_line.qnty, l_line.unit_price,
        ...
        ON ACTION accept ATTRIBUTES(TEXT="Save Line", IMAGE="save")
            ...
            IF p_is_new THEN
                ...
                LET m_arr_ord_lines[m_arr_ord_lines.getLength()].vat_amt = l_line.vat_amt
                LET m_arr_ord_lines[m_arr_ord_lines.getLength()].line_total = l_line.line_total
            ELSE
                -- Update full array
                ...
                LET m_arr_ord_lines[p_row].vat_amt = l_line.vat_amt
                LET m_arr_ord_lines[p_row].line_total = l_line.line_total
            END IF
...
FUNCTION get_available_stock(p_stock_id INTEGER)
    ...
    -- Get stock on hand
    SELECT stock_on_hand INTO l_on_hand
      FROM st01_mast
     WHERE stock_id = p_stock_id
    ...
    SELECT COALESCE(SUM(det.qnty), 0) INTO l_reserved
...
FUNCTION check_stock_availability(p_stock_id INTEGER, p_required_qty DECIMAL,
    ...
    IF l_available >= p_required_qty THEN
        LET l_ok = TRUE
        LET l_message = SFMT("Stock available: %1", l_available USING "<<<,<<<,<<&.&&")
    ELSE
        LET l_ok = FALSE
        LET l_message = SFMT("Insufficient stock for %1\nRequired: %2 | Available: %3 | Short: %4",
                            l_stock_desc,
...
FUNCTION validate_order_stock_levels(p_order_id INTEGER)
    ...
    -- Check each line
    FOR i = 1 TO m_arr_ord_full_lines.getLength()
        IF m_arr_ord_full_lines[i].stock_id IS NOT NULL AND
           m_arr_ord_full_lines[i].qnty > 0 THEN

            CALL check_stock_availability(
                m_arr_ord_full_lines[i].stock_id,
                m_arr_ord_full_lines[i].qnty,
                p_order_id)
                RETURNING l_ok, l_on_hand, l_available, l_message

            IF NOT l_ok THEN
                LET l_has_errors = TRUE
                LET l_error_msg = l_error_msg || SFMT("Line %1: %2\n",
                                                       i, l_message)
            END IF
        END IF
    END FOR
...
FUNCTION delete_order_line(p_doc_id INTEGER, p_row INTEGER)
    IF p_row > 0 THEN
        IF utils_globals.show_confirm(
            ...
            CALL m_arr_ord_lines.deleteElement(p_row)
            CALL m_arr_ord_full_lines.deleteElement(p_row)
...
FUNCTION save_order_lines(p_doc_id INTEGER)
    ...
    CALL validate_order_stock_levels(p_doc_id)
        RETURNING l_stock_ok, l_stock_msg
    ...
    TRY
        ...
        FOR i = 1 TO m_arr_ord_full_lines.getLength()
            ...
            LET m_arr_ord_full_lines[i].hdr_id = p_doc_id
            INSERT INTO sa31_ord_det VALUES m_arr_ord_full_lines[i].*
        END FOR
...
FUNCTION calculate_order_totals()
    ...
    FOR i = 1 TO m_arr_ord_full_lines.getLength()
        LET l_gross = l_gross + NVL(m_arr_ord_full_lines[i].gross_amt, 0)
        LET l_disc_tot = l_disc_tot + NVL(m_arr_ord_full_lines[i].disc_amt, 0)
        LET l_vat_tot = l_vat_tot + NVL(m_arr_ord_full_lines[i].vat_amt, 0)
    END FOR
...
-- ==============================================================
PRIVATE FUNCTION load_customer_details(p_acc_code INTEGER)
...
FUNCTION load_order(p_doc_id INTEGER)
    ...
    IF SQLCA.SQLCODE = 0 THEN
        ...
        -- ===========================================
        LET l_can_edit = can_edit_order(m_rec_ord.id, m_rec_ord.status)
        ...
        FOREACH ord_lines_cur
            INTO m_arr_ord_lines[idx + 1].stock_id,
                 m_arr_ord_lines[idx + 1].batch_id,
                 m_arr_ord_lines[idx + 1].qnty,
                 m_arr_ord_lines[idx + 1].unit_price,
                 m_arr_ord_lines[idx + 1].sell_price,
                 m_arr_ord_lines[idx + 1].disc_amt,
                 m_arr_ord_lines[idx + 1].vat_amt,
                 m_arr_ord_lines[idx + 1].line_total
            LET idx = idx + 1
        END FOREACH
        ...
        FOREACH ord_full_lines_cur INTO m_arr_ord_full_lines[idx + 1].*
            LET idx = idx + 1
        END FOREACH
        ...
        DISPLAY ARRAY m_arr_ord_lines TO arr_sa_ord_lines.*
            ...
            ON ACTION edit ATTRIBUTES(TEXT = "Edit Order", IMAGE = "pen")
                IF NOT l_can_edit THEN
                    CALL utils_globals.show_error(
                        "Cannot edit order with status: " || m_rec_ord.status)
                    CONTINUE DISPLAY
                END IF
            ...
            ON ACTION copy_to_invoice ATTRIBUTES(TEXT="Copy to Invoice", IMAGE="forward")
                CALL copy_order_to_invoice(p_doc_id)
                -- Reload to show updated status
                EXIT DISPLAY

            ON ACTION view_invoice ATTRIBUTES(TEXT="View Invoice", IMAGE="info")
                CALL view_linked_invoice(m_rec_ord.id)
...
-- ==============================================================
FUNCTION can_edit_order(p_order_id INTEGER, p_status VARCHAR(10))
    RETURNS SMALLINT

    DEFINE l_invoice_count INTEGER

    -- Check status
    IF p_status = "INVOICED" OR p_status = "COMPLETED" OR p_status = "CANCELLED" THEN
        RETURN FALSE
    END IF

    -- Check for linked invoices
    SELECT COUNT(*) INTO l_invoice_count
      FROM sa32_inv_hdr
     WHERE ref_doc_type = "ORDER"
       AND ref_doc_no = (SELECT doc_no FROM sa31_ord_hdr WHERE id = p_order_id)
...
FUNCTION copy_order_to_invoice(p_order_id INTEGER)
    ...
    IF l_invoice_count > 0 THEN
        CALL utils_globals.show_error("Order has already been converted to an invoice")
        RETURN
    END IF

    -- Load order
    SELECT * INTO l_order_hdr.*
      FROM sa31_ord_hdr
     WHERE id = p_order_id
    ...
    TRY
        ...
        SELECT COALESCE(MAX(id), '0') + 1 INTO l_new_invoice_doc_no
          FROM sa32_inv_hdr
...
FUNCTION view_linked_invoice(p_order_doc_no INTEGER)
    ...
    SELECT id, doc_no INTO l_invoice_id, l_invoice_doc_no
      FROM sa32_inv_hdr
     WHERE ref_doc_type = "ORDER"
       AND ref_doc_no = p_order_doc_no
...
FUNCTION save_order()
    ...
    BEGIN WORK
    TRY
        SELECT COUNT(*) INTO exists FROM sa31_ord_hdr WHERE id = m_rec_ord.id

        IF exists = 0 THEN
            INSERT INTO sa31_ord_hdr VALUES m_rec_ord.*
            CALL utils_globals.show_success("Order saved")
        ELSE
            UPDATE sa31_ord_hdr
                SET doc_no = m_rec_ord.id,
        ...
        -- Save lines
        CALL save_order_lines(m_rec_ord.id)
...
FUNCTION delete_order(p_doc_id INTEGER)
    ...
    IF l_invoice_count > 0 THEN
        CALL utils_globals.show_error(
            SFMT("Cannot delete order - it has %1 linked invoice(s)", l_invoice_count))
        RETURN
    END IF
