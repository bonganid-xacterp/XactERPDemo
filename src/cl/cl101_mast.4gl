...
FUNCTION init_cl_module()
    ...
    DISPLAY ARRAY m_cred_trans_arr TO m_cred_trans_arr.*
        ...
        ON ACTION New        CALL new_creditor();   LET is_edit_mode = FALSE
        ...
        ON ACTION List       CALL load_all_creditors(); LET is_edit_mode = FALSE
        ON ACTION Edit       IF m_cred_rec.id IS NULL OR m_cred_rec.id = 0 THEN CALL utils_globals.show_info("No record selected to edit.") ELSE LET is_edit_mode = TRUE; CALL utils_globals.set_form_label('lbl_form_title', 'CREDITORS MAINTENANCE'); CALL edit_creditor() END IF
        ON ACTION DELETE     CALL delete_creditor(); LET is_edit_mode = FALSE
        ON ACTION PREVIOUS   CALL move_record(-1)
        ON ACTION Next       CALL move_record(1)
        ON ACTION add_order  IF m_cred_rec.id THEN CALL pu130_order.new_po_from_master(m_cred_rec.id) ELSE CALL utils_globals.show_warning('Choose a creditor record first.') END IF
