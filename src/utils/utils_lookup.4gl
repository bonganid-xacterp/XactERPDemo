...
FUNCTION lookup_creditor(search_val STRING, field_name STRING) RETURNS STRING
...
FUNCTION lookup_stock(search_val STRING, field_name STRING) RETURNS STRING

    DEFINE sql_query STRING
    LET sql_query =
        "SELECT stock_code, description, unit, selling_price, category.name "
            || "FROM st01_mast "
...
FUNCTION lookup_stock_category(
    ...
    DEFINE sql_query STRING
    LET sql_query =
        "SELECT cat_code, cat_name, description, NULL, NULL "
            || "FROM st_cat "
...
FUNCTION build_where(
    search_val STRING, field1 STRING, field2 STRING)
    RETURNS STRING
...
FUNCTION do_lookup(
    sql_query STRING, title STRING, field_name STRING)
    RETURNS STRING
    ...
    # Run query + dialog
    CALL execute_lookup_query(sql_query, title) RETURNING results, selected_idx

    IF selected_idx > 0 THEN
        CASE field_name.toUpperCase()
            WHEN "CODE"
                LET return_val = results[selected_idx].code
            WHEN "DESC"
                LET return_val = results[selected_idx].description
            WHEN "FIELD3"
                LET return_val = results[selected_idx].field3
            WHEN "FIELD4"
                LET return_val = results[selected_idx].field4
            WHEN "FIELD5"
                LET return_val = results[selected_idx].field5
            OTHERWISE
                LET return_val = results[selected_idx].code
        END CASE
    END IF
...
# ----------------------------------------------------------------------------
FUNCTION execute_lookup_query(
    sql_query STRING, module_title STRING)
    ...
    CALL display_lookup_dialog(
        result_array, row_count, module_title)
...
# ----------------------------------------------------------------------------
FUNCTION display_lookup_dialog(
    result_array DYNAMIC ARRAY OF t_lookup_result,
    row_count INTEGER,
    dialog_title STRING)
    RETURNS INTEGER
